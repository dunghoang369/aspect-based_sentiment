...
class SpanModelDocument(BaseModel):
    ...
    def from_sentence(cls, x: Sentence):
        ...
        relations = []
        for t in x.triples:
            if t.o_start == t.t_start and t.o_end == t.t_end:
                #ner.append((t.t_start, t.t_end, LabelEnum.target))
                ner.append((t.o_start, t.o_end, LabelEnum.opinion))
                #relations.append((t.o_start, t.o_end, -1, -1, t.label))
                #relations.append((t.o_start, t.o_end, t.t_start, t.t_end, t.label))
                #print (x.tokens)
                #print (relations)
                #print ("-----------------")
            else:
...
class SpanModelPrediction(SpanModelDocument):
    ...
    def to_sentence(self) -> Sentence:
        ...
        #with open("/app/src/Span-ASTE/scores.txt", "a") as f:
        #    if len(scores) > 0:
        #        f.write(str(scores) + "\n")
        #    else:
        #        f.write("None" + "\n")
...
class SpanModel(BaseModel):
    ...
    def predict(self, path_in: str, path_out: str):
        ...
        with open(path_temp_out) as f:
            ...
            for line in f:
                output = SpanModelPrediction(**json.loads(line.strip()))
                pred, score, ner = output.to_sentence()
                preds.append(pred)
                scores.append(score)
                ners.append(ner)
        print("--------------------------------", preds)
